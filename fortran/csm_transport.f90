module csm_transport
 ...
 use get_vals, only: op, query_csm_density, query_csm_inner_edge, query_csm_outer_edge, &
                     query_tau_to_edge, query_csm_velocity, &
...
contains
 ...
 real(8) :: old_inner, old_outer, e_removed
  ...
  call update_interaction_geometry(state, r_shell, t_shell, m_shell)
  call remap_y_conservative(old_r, old_inner, old_outer, old_y, state%n_zones, &
                            state%radius, state%r_inner, state%r_outer, state%y, state%n_zones, e_removed)
  state%e_shocked_store = state%e_shocked_store + e_removed
  call fill_profile_arrays(state, t_shell)
  call update_tau_and_luminosity(state)
...
subroutine shell_structure_estimate(state, r_shell, t_shell, m_shell, delta_r, tau_sh, tleak, m_csm_shell, m_ej_shell)
  ...
  ! The post-shock shell is radiative in the thin-shell regime, so the cooled
  ! compression can greatly exceed the adiabatic strong-shock value of 4.
  ! Use an isothermal-style Mach^2 estimate with a conservative cap to avoid an
  ! artificially thick cooling shell that suppresses breakout luminosity.
  comp_csm = min(1d5, max(4d0, (max(u_eff - v_csm, 0d0) / c_s_cold)**2))
  comp_ej = min(1d5, max(4d0, (max(v_ej - u_eff, 0d0) / c_s_cold)**2))
...
subroutine interaction_transport_step(state, dt, r_shell, u_shell, t_shell, m_shell, lum_heat, lum_obs, r_ph)
  ...
  call solve_transport_step(state, dt, max(lum_heat,0d0))
...
subroutine initialize_cooling_state_from_interaction(state, r_shell, u_shell, m_shell, t_shell)
 type(transport_state_type), intent(inout) :: state
  ...
  integer :: i
  real(8), allocatable :: old_r(:), old_rho(:), old_y(:)
  real(8) :: r_face_l, r_face_r, vol_i, e_old, e_new, renorm_e
  real(8) :: old_inner, old_outer

  if (.not. state%initialized) return
  allocate(old_r(state%n_zones), old_rho(state%n_zones), old_y(state%n_zones))
  old_r = state%radius
  old_rho = max(state%rho, 1d-30)
  old_y = max(state%y, 1d-40)
  old_inner = state%r_inner
  old_outer = state%r_outer
  ...
  ! Preserve the interaction-phase transport field through breakout instead of
  ! remapping it onto a guessed compressed shell. The remap creates the
  ! artificial post-peak collapse seen in the compact case.
  state%r_emerge_inner = old_inner
  state%r_emerge_outer = old_outer
  state%cooling_scale = 1d0
  ...
  state%r_outer = max(state%r_emerge_outer, state%r_emerge_inner*(1d0 + 1d-6))
  state%radius = old_r
  state%rho = old_rho
  ! Include energy swept below the moving interaction boundary; after breakout
  ! it belongs to the shocked shell and should seed the cooling phase.
  e_old = max(state%e_shocked_store, 0d0)
  do i = 1, state%n_zones
   call zone_geometry_from_array(old_r, old_inner, old_outer, state%n_zones, i, r_face_l, r_face_r, vol_i)
   e_old = e_old + a_rad * old_y(i) * vol_i
  end do
  state%y = old_y
  e_new = 0d0
  do i = 1, state%n_zones
   call zone_geometry(state, i, r_face_l, r_face_r, vol_i)
   e_new = e_new + a_rad * state%y(i) * vol_i
  end do
  if (e_new > 0d0) then
   renorm_e = e_old / e_new
   state%y = max(state%y * renorm_e, 1d-40)
  end if
  state%radius_ref = state%radius
  state%rho_ref = state%rho
  state%e_shocked_store = 0d0
  call update_tau_and_luminosity(state)
  deallocate(old_r, old_rho, old_y)
...
subroutine solve_transport_step(state, dt, lum_heat)
 ...
 integer :: i, n, ish, ilo, ihi
  ...
  real(8) :: shock_w, frac_hi, frac_lo
  ...
  if (.not. state%in_cooling_phase) then
   if (state%r_shell_current <= state%radius(1)) then
    ilo = 1
    ihi = 1
   else if (state%r_shell_current >= state%radius(n)) then
    ilo = n
    ihi = n
   else
    do i = 2, n
     if (state%r_shell_current <= state%radius(i)) then
      ilo = i-1
      ihi = i
      exit
     end if
    end do
    frac_hi = (state%r_shell_current - state%radius(ilo)) / &
              max(state%radius(ihi) - state%radius(ilo), 1d-30)
    frac_hi = min(max(frac_hi, 0d0), 1d0)
    frac_lo = 1d0 - frac_hi
   end if
  end if
  ...
  do i = 1, n
   b(i) = a_rad * vol_arr(i) / dt + 0.5d0 * (kl_arr(i) + kr_arr(i) + kesc_arr(i))
   rhs(i) = a_rad * vol_arr(i) * old_y(i) / dt - &
            0.5d0 * (kl_arr(i) + kr_arr(i) + kesc_arr(i)) * old_y(i)

   if (i > 1) then
    a(i) = -0.5d0 * kl_arr(i)
    rhs(i) = rhs(i) + 0.5d0 * kl_arr(i) * old_y(i-1)
   end if
   if (i < n) then
    c(i) = -0.5d0 * kr_arr(i)
    rhs(i) = rhs(i) + 0.5d0 * kr_arr(i) * old_y(i+1)
   end if

   shock_w = 0d0
   if (state%in_cooling_phase) then
    if (i == ish) shock_w = 1d0
   else
    if (i == ilo) shock_w = shock_w + frac_lo
    if (i == ihi) shock_w = shock_w + frac_hi
   end if
   if (shock_w > 0d0) then
    rhs(i) = rhs(i) + max(lum_heat, 0d0) * shock_w
   end if
  end do
